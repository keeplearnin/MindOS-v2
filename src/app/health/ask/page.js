...
import { useHealthQueries, askHealthQuestion, createProtocol, useKnowledgeSources, useKnowledgeVideos, useKnowledgeArticles, useVideoStats, loadSource, loadTranscripts } from '@/lib/health-hooks';
import { MessageSquare, Send, Loader2, ExternalLink, Globe, FlaskConical, ChevronDown, ChevronUp, Clock, AlertTriangle, Library, Plus, CheckCircle2, AlertCircle, Youtube, FileText, Play, Share2 } from 'lucide-react';
      ...
      {!currentAnswer && queries.length === 0 && !queriesLoading && (
        <div className="card text-center py-12">
          ...
          <p className="text-sm" style={{ color: 'var(--text-muted)' }}>
            Add and index a source above first, then ask away.
          </p>
        </div>
      )}
    </div>
...
// ─── Sources (merged from the old Sources page) ───────────────
// Collapsible: source management is occasional; asking is the daily act.
// Auto-opens when the library is empty, since Ask is useless without it.
function SourcesSection() {
  ...
  const isEmpty = !sourcesLoading && sources.length === 0;
  ...
  return (
    <div className="mt-8">
      ...
      >
        <Library size={16} style={{ color: 'var(--text-muted)' }} />
        <span className="text-base font-semibold">Sources</span>
        {!sourcesLoading && (
          <span className="text-xs px-1.5 py-0.5 rounded-full" style={{ background: 'var(--bg)', color: 'var(--text-muted)', border: '1px solid var(--border)' }}>
            {sources.length}
          </span>
        )}
        <span className="ml-auto">
          {showBody ? <ChevronUp size={16} style={{ color: 'var(--text-muted)' }} /> : <ChevronDown size={16} style={{ color: 'var(--text-muted)' }} />}
        </span>
      </button>
        ...
        <>
          <div className="card mb-4">
            <p className="text-sm mb-3" style={{ color: 'var(--text-muted)' }}>
              Paste any URL — YouTube channel, video, or article. Content is indexed for Q&A above.
            </p>
          ...
          {sourcesLoading ? (
            ...
            <p className="text-sm text-center py-4" style={{ color: 'var(--text-muted)' }}>
              No sources yet — add one above to start asking questions.
            </p>
          ) : (
            <div className="space-y-3">
              {sources.map(source => (
